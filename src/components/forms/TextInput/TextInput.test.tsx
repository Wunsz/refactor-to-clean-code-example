...
describe('components / forms / raw / <TextInput/>', () => {
  it('should render default component with label', () => {
    const { getByLabelText } = render(<TextInput label="label" value="some text" onChange={vi.fn()} />);
  ...
  it('firing text change on input should trigger onChange', () => {
    ...
    const { getByLabelText } = render(
      <TextInput label="label" value="some text" onChange={(event) => changeHandler(event.target.value)} />,
    );
